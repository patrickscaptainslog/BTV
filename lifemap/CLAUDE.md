...
- **The galaxy is the ritual, not the interface.** Retrieval is a ⌘K search overlay (boring, instant); the search *result* is a camera flight to the star. The galaxy is the reward surface you return to — an evening sky that replays your day — not the working surface you fight through.
...
Keep the aesthetic cinematic and restrained: glassmorphism panels, no chrome, labels only on hover/select. When in doubt, more depth and motion quality, not more UI. **Beauty is frame rate**: instanced particles, device-tiered particle caps, bloom doing the perceptual heavy lifting — motion quality over particle count, 60fps over spectacle.
